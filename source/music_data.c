...
void
raw_to_music_data ( Raw_music_data raw_music_data, Music_data *music_data )
{
...
// for quick sort we need two functions
int
sort_compare ( Music_data music_data, int i, int j )
{
...
// swaps the value of two music_data entries
void
sort_swap ( Music_data *music_data, int i, int j )
{
  if (i == j)
    return;
...
void
quick_sort_music_data ( Music_data *music_data, int low, int high )
{
  if (low < high) {
    int pivot = high;
    int i = (low - 1);
...
void
sort_music_data ( Music_data *music_data )
{
  quick_sort_music_data(music_data, 0, music_data->size -1);
}

bool fast_strcmp (const char *a, const char *b) {
  if (a == NULL || b == NULL) {
    return false;
  }
...
Music_data_index
get_index_music_data ( const char *in, Music_data music_data )
{
  Music_data_index out;
...
void
free_raw_music_data ( Raw_music_data *music_data )
{
  free(music_data->id);
...
void
free_music_data ( Music_data *music_data )
{
  for (int i = 0; i < music_data->size; i++) {
    for (int j = 0; j < music_data->album_size[i]; j++) {
